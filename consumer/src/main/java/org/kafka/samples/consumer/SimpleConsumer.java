package org.kafka.samples.consumer;
...
public class SimpleConsumer {
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			System.out
					.println("SimplesimpleConsumerZkConsumer {zookeeper} {group.id} {topic} {window-size} {wait-time}");
			return;
		}
		...
		// simpleConsumer.waitTime = args[4];

		CircularFifoBuffer buffer = new CircularFifoBuffer(window);

		simpleConsumer.configure(zkUrl, groupId);

		simpleConsumer.start(topic);

		while ((next = simpleConsumer.getNextMessage()) != null) {
			int sum = 0;
			...
			// simpleConsumer.consumer.commitOffsets();

		}

		simpleConsumer.consumer.shutdown();
		System.exit(0);
	...
	private void start(String topic) {
		...
		/*
		 * We tell Kafka how many threads will read each topic. We have one
		 * topic and one thread
		 */
		...
		/*
		 * Kafka will give us a list of streams of messages for each topic. In
		 * this case, its just one topic with a list of a single stream
		 */
		stream = consumer.createMessageStreams(topicCountMap, decoder, decoder)
				.get(topic).get(0);
	}
